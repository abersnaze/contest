...
for (a, b), dist in sorted_dists[:cloest]:
    ca = circuits[a]
    cb = circuits[b]
    if ca == cb:
        continue
...
unique_circuits = {c.seed: c for c in circuits.values()}

sizes = sorted(
    list(map(lambda c: len(c.boxes), unique_circuits.values())), reverse=True
)
print(sizes)

print(math.prod(sizes[:largest]))
